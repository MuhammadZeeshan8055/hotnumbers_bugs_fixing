...
function filter_shipping_rules($cart = []) {
    ...
    // Reset the shipping adjustments and rules before processing
    unset($cart['shipping_add']);
    unset($cart['shipping_rule']);
    ...
    foreach ($shipping_rules as $rules) {
        ...
        $condition_match = [];
        $all_conditions_met = true; // Assume all conditions are met initially

        foreach ($rules['option_data'] as $rule) {
            $subject = $rule['subject'];
            $condition = $rule['condition'];
            $value = $rule['value'];
            $condition_met = false; // Assume condition is not met

            // Check each condition type
            if ($subject === "user") {
                if ($condition === 'equal_to' && $user_id === $value) {
                    $condition_met = true;
                } elseif ($condition === 'not_equal_to' && $user_id !== $value) {
                    $condition_met = true;
                }
            }
            ...
            if ($subject === "user_role") {
                ...
                if (!empty($db_role)) {
                    $db_role = $db_role[0]->role;
                    if ($condition === 'equal_to' && array_key_exists($db_role, $role)) {
                        $condition_met = true;
                    } elseif ($condition === 'not_equal_to' && !array_key_exists($db_role, $role)) {
                        $condition_met = true;
                    }
                }
            }
            ...
            if ($subject === "product_category" && !empty($cart['products'])) {
                foreach ($cart['products'] as $product) {
                    ...
                    if ($condition_met) {
                        break; // Break if any product matches the category condition
                    }
                }
            }
            ...
            // If a condition isn't met, set flag to false and break
            if (!$condition_met) {
        ...
        // If all conditions are met for a rule, apply the shipping rule
        if ($all_conditions_met) {
            ...
            // Break after applying the first matched rule, or comment this out if you want to apply multiple rules
            break;
        }
    }
