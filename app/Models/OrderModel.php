...
class OrderModel extends Model
{
    ...
    public function generate_order_slip($order_ids = [])
    {
        ...
        if ($length < 2) {
            foreach ($slips as $index => $slip) {
                $customerName = $slip['customer']->fname .''. $slip['customer']->lname ?? 'Unknown Customer';
                $orderId = $slip['order_id'] ?? 'Unknown Order ID';

                // Loop through each page and assign customer and order ID text
                for ($i = 1; $i <= $pageCount; $i++) {
                    if ($i === 1) {
                        // Add text for the current order on the current page (only for the first page)
                        $canvas->page_text(
                            100, // X position (from the left)
                            790, // Y position (from the top)
                            "Customer: {$customerName} | Order ID: {$orderId}", // Text to display
                            null, // Font
                            10,   // Font size
                            [0, 0, 0] // Color in RGB
                        );
    ...
    {
        $csv_array = [
            ['Order ID', 'Order Date', 'Order Status', 'Payment Method', 'Order Type', 'Customer Name (Billing)', 'Customer Role', 'Item name', 'Category', 'Item cost', 'Quantity', 'Item total', 'Shipping', 'Discount', 'Subtotal']
        ];
            ...
            if (!empty($order)) {
                ...
                foreach ($order_items as $j => $item) {
                    ...
                    if (!empty($item_meta['product_id'])) {
                        ...
                        $row_array[7] = $productName; // Item name
                        $row_array[8] = implode(', ', $productCatList); // Category
                        $row_array[9] = _price((float)$item_meta['item_price']); // Item cost
                        $row_array[10] = (int)$item_meta['quantity']; // Quantity
                        $row_array[11] = $line_subtotal; // Item total

                        if ($j > 0) {
                            // Emptying unnecessary columns for multiple products in the same order
                            $row_array[0] = '';
                            $row_array[1] = '';
                            $row_array[2] = '';
                            $row_array[3] = '';
