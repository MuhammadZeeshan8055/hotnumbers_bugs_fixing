...
class ProductsModel extends Model
{
    ...
    public function product_variation_for_edit($product_id = 0, $where = [], $filter_price = 1)
    {
        $sql = "SELECT id, variation FROM tbl_product_variations WHERE product_id='$product_id' ";
        $row = $this->db->query($sql)->getRow();
        ...
        if (!empty($row)) {
            $values = [];
            if (!empty($where)) {
                // Remove 'variation_' prefix from $where keys
                $where = array_combine(
                    array_map(function ($key) {
                        return str_replace('variation_', '', $key);
                    }, array_keys($where)),
                    array_values($where)
                );
                ...
                foreach ($variations as $variation) {
                    if (!empty($variation['keys'])) {
                        ...
                        $var_keys = $variation['keys'];
                        $var_keys = array_filter($var_keys); // Remove empty keys

                        // Compare only non-empty keys in $where with $var_keys
                        $match = true;
                        foreach ($where as $key => $value) {
                            // Only compare if the key exists in $var_keys and is not empty
                            if (isset($var_keys[$key]) && $var_keys[$key] !== $value) {
                                $match = false;
                                break;
                            }
                        }
    ...
    public function variation_gain_stock($product_id, $stock_deduct = 0, $orderID)
    {
