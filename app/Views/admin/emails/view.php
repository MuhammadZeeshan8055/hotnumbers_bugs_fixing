            ...
            <fieldset>
                ...
                <div class="row mt-10">
                    <div class="col-md-4">
                        <div class="input_field">
                            <label>From:</label>
                            <a href="<?php echo admin_url() ?>settings#mail-smtp" style="font-size: 12px;color: var(--red);text-decoration: underline;" class="pull-right">SMTP Settings</a>
                            <?php /*<input name="from" value="<?php echo $email['mail_from'] ?>" placeholder="<?php echo $setting['online_admin_email'] ?>">*/ ?>
                            <div>
                                <select class="select2" data-search="false" name="smtp_id" value="<?php echo !empty($email['smtp_id']) ? $email['smtp_id'] : 0 ?>">
                                    <?php
                                    if(!empty($smtp_settings)) {
                                    foreach($smtp_settings as $setting) {
                                        ?>
                                        <option value="<?php echo $setting->id ?>"><?php echo $setting->username ?></option>
                                        <?php
                                    }
                                    }
                                    ?>
