...
class AdminController extends BaseController
{
    ...
    public function settings()
    {
        ...
        if($this->request->getGet('del_smtp')) {
            $id = $this->request->getGet('del_smtp');
            $this->master->delete_data('tbl_smtp_settings','id',$id);
            notice_success('SMTP record deleted successfully');
            return redirect()->to(admin_url().'admin/settings');
        }

        if ($post_data = $this->request->getPost()) {

            if(!empty($post_data['smtp_test'])) {
                $to_email = $post_data['test_email'];
                $test_message = '<div><br><br>'.$post_data['test_message'].'</div>';
                $smtp_id = $post_data['smtp_test'];
                $mailModel->subject("SMTP Test Email");
                $sent = $mailModel->send_email($to_email,$test_message,['smtp_id'=>$smtp_id]);
                if($sent) {
                    echo json_encode(['success'=>1,'message'=>'Test email sent successfully']);
                }else {
                    echo json_encode(['success'=>0,'message'=>'Failed to send test email']);
                }
                exit;
            }
            ...
            if(!empty($post_data['subscriptionForm'])) {
                ...
                if(!empty($post_data['subscriptionForm']['duration']['name'])) {
            ...
            if(!empty($post_data['shipment_rule'])) {
                foreach($post_data['shipment_rule'] as $idx=>$rules) {
                    $rules_data = [];
                    if(!empty($rules['subject'])) {
                            ...
                            $rules_data = $option_data;
                    }

                    $post_data['shipment_rule'][$idx]['option_data'] = $rules_data;
                    unset($post_data['shipment_rule'][$idx]['subject']);
                    unset($post_data['shipment_rule'][$idx]['condition']);
                    unset($post_data['shipment_rule'][$idx]['value']);
                }
            }
            ...
            if(!empty($post_data['mail_smtp'])) {
                foreach($post_data['mail_smtp']['host'] as $i=>$host) {
                    $id = !empty($post_data['mail_smtp']['id'][$i]) ? $post_data['mail_smtp']['id'][$i] : 0;
                    $port = $post_data['mail_smtp']['port'][$i];
                    $mail_from = $post_data['mail_smtp']['mail_from'][$i];
                    $username = $post_data['mail_smtp']['username'][$i];
                    $password = $post_data['mail_smtp']['password'][$i];
                    $db_data = [
                      'host' => $host,
                      'port' => $port,
                      'mail_from' => $mail_from,
                    ...
                    $this->master->insertOrUpdate('tbl_smtp_settings',$db_data,'id',$id);
                }
            }else {
        ...
        $this->data['smtp_settings'] = $this->master->getRows('tbl_smtp_settings');
    ...
    public function pass_req() {
        ...
        if ($data = $this->input->post()) {
            ...
            if ($row['id'] != '') {
                $link = base_url(ADMIN) . "/reset_password/".$row['forget_code'];
