...
class Orders extends MyController
{
    ...
    public function add_order($order_id = 0)
    {
        ...
        if (!empty($this->request->getPost('products'))) {
            ...
            $shippingCost = !empty($shippingCost) ? (float) $shippingCost : 0;
            $order_tax = !empty($order_tax) ? (float) $order_tax : 0;
            ...
            // $user_discounted_total = $subtotal * ($user_discount / 100);

            if (is_numeric($user_discount) && $user_discount > 0) {
                $user_discounted_total = $subtotal * ($user_discount / 100);
            } else {
                $user_discounted_total = 0; // No discount applied
            }

            if (is_numeric($wholesale_discount) && $wholesale_discount > 0) {
                $wholesale_discounted_total = $subtotal * ($wholesale_discount / 100);
            } else {
                $wholesale_discounted_total = 0; // No discount applied
            }
            ...
            if (isset($wholesale_discount)) {
                $order_meta['wholesale_discount'] = $wholesale_discounted_total;
            ...
            if ($user_discount_type == 'fixed') {
                ...
                $order_meta['user_discount'] = $user_discounted_total;
                $order_meta['user_discount_text'] = $user_discount;
                $order_meta['user_discount_type'] = $user_discount_type;
            }
            ...
            if (!empty($coupon)) {
                ...
                $order_meta['coupon_discount'] = $coupon_discount;
            } else {
                $order_meta['coupon_id'] = 0;
                $order_meta['coupon_code'] = "";
                $order_meta['coupon_discount'] = "";
            }

            if(!empty($orderShipping)){

                $cartModel = model('CartModel');
                $shipping_values=$cartModel->shipping_method_by_idx($orderShipping);

                $order_meta['shipping_name'] = $shipping_values['name'];	
                $order_meta['shipping_cost'] = $shipping_values['amount'];
                $order_meta['order_shipping_title'] = $shipping_values['name'];
                $order_meta['order_shipping'] = $shipping_values['amount'];

                $order_items_shipping = [
                    'order_id' => $order_id,
                    'product_name' => $shipping_values['name'], // Shipping product name
                    'item_type' => 'shipping', // Item type for shipping
                ];

                $masterModel->insertData('tbl_order_items', $order_items_shipping);

                $order_items_tax = [
                    'order_id' => $order_id,
                    'product_name' => 0, // tax product name
                    'item_type' => 'tax', // Item type for tax
                ];

                $masterModel->insertData('tbl_order_items', $order_items_tax);
                ...
                if (!empty($order_items)) {
                    ...
                    foreach ($order_items as $item) {
                        if (!empty($item['item_meta'])) {
                            ...
                            $ProductsModel->variation_gain_stock($item_meta['product_id'], $item_meta['quantity'], $order_id);
