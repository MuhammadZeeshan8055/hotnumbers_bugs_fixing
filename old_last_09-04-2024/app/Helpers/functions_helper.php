...
function transaction_link($link_id='', $meta_data=[]) {
    if($meta_data['payment_method'] === "stripe") {
        //Stripe
    ...
    if($meta_data['payment_method'] === "squareup") {
        //Stripe
    ...
    if($meta_data['payment_method'] === "braintree") {
        //Braintree
        if(!empty($meta_data['transaction_link'])) {
            return '<a target="_blank" class="color-base btn btn-sm btn-primary" href="' . $meta_data['transaction_link'] . '">Braintree payment details</a>';
        }
    }
    if($meta_data['payment_method'] === "direct") {
...
function is_logged_in() {
    ...
    if(!empty($user['id'])) {
        $uid = $user['id'];
        $is_user = $master->query("SELECT user_id FROM tbl_users WHERE user_id='$uid'",false,true);
        if(empty($is_user)) {
            return false;
        }
        return $uid;
...
function is_customer() {
    ...
    if(!empty($user['id'])) {
        ...
        $data = $userModel->get_user($uid,'status');
        if((empty($user_roles) || in_array('customer',array_keys($user_roles))) && $data->status == 1) {
            $is_user = $uid;
        }
    }
    return $is_user;
...
function current_user_role($include_meta=[]) {
    if(is_logged_in()) {
        $master = model('MasterModel');
        $uid = is_logged_in();
        $get_user = $master->query("SELECT role.* FROM tbl_user_roles AS role JOIN tbl_users AS user ON user.role=role.id WHERE user.user_id='$uid'",true,true);
        return $get_user;
    }else {
        return [
            'id' => 9,
...
{
    set_message('sessionmessage', $message, 'board_notice success', $type);
}
...
function set_message($key = 'success', $message='', $mode = 0,$type='toast', $timer = 5000)
{
    $session = session();
...
function get_message($k='sessionmessage')
{
    $session = session();
    ...
    if (!empty($message)) {
        ...
        if($message['msg_type'] == 'toast') {
            ...
            return '<div class="notice-board notification_message type-'.$message['type'].'">'.$message['message'].'</div>';
        }
    }
}
...
function get_setting($title='',$array=false) {
    $db = db_connect();
    $row = $db->query("select value from tbl_settings where title='$title'");
    $row = $row->getRow();
    if(!empty($row->value)) {
        if(!$array) {
            return $row->value;
    ...
    /*if($allowed && !$ignore_permission) {
        ...
        if (!empty($db_tax_rate)) {
            ...
            foreach ($db_tax_rate as $rate) {
                ...
                array_walk($rate, function(&$value)
                {
                    $value = strtolower($value);
                });
            ...
            if(empty($tax_rate)) {
                foreach ($db_tax_rate as $rate) {
                    if($rate['country'] === "" && $rate['postcode'] === "" && $rate['state'] === "" && $rate['city'] === "") {
                        $tax_rate = $rate;
                    }
                }
            }
            ...
            if(json_decode($tax_class->value)) {
                ...
                if(!empty($tax_arr['country'])) {
                    foreach($tax_arr['country'] as $i=>$tax_country) {
                        ...
                        $tax_shipping = $tax_arr['tax_shipping'][$i];
...
function get_tax_price($tax_rate_arr=[]) {

    if(empty($tax_rate_arr)) {
...
function upload_media_box($config=[],$include_scripts=true) {
    ...
    <div class="input_file upload_media_images <?php echo !empty($config['multiple']) ? 'multiple':'' ?>">
        ...
        <div>
            ...
            <a href="#" <?php echo $atts ?> class="btn btn-sm btn-primary back browse_media"><?php echo empty($config['buttonText']) ? '<i class="icon-up-circled2"></i> Upload image':$config['buttonText'] ?></a>
                ...
                fetch(url).then(res => res.json()).then((res) => {
                    if (Object.keys(res).length) {
                        for (let k in res) {
                            ...
                            plan_expire.find('select').append(`<option value="${k}">${res[k]}</option>`);
                        }
