...
//		$this->CI->load->config('paypallib_config');
		...
		$sanbox = env('paypal_sandbox');
		$this->paypal_url = ($sanbox == TRUE)?'https://www.sandbox.paypal.com/cgi-bin/webscr':'https://www.paypal.com/cgi-bin/webscr';
                
		$this->last_error = '';
		$this->ipn_response = '';

		$this->ipn_log_file = env('paypal_lib_ipn_log_file');

		$this->ipn_log = env('paypal_lib_ipn_log');
		
		$this->button_path = env('paypal_lib_button_path');

		$this->return_url = env('paypal_return');

		$this->paypal_notify = env('paypal_notify');
		...
		$businessEmail = env('paypal_business');
		$this->add_field('business',$businessEmail);
		//$this->add_field('rm','2');
		// Return method = POST
		//$this->add_field('cmd','_cart');
		//$this->add_field('upload',1);
		$this->add_field('return',base_url($this->return_url));
		$this->add_field('notify_url',base_url($this->paypal_notify));

		$this->add_field('currency_code', env('paypal_lib_currency_code'));
		$this->button('Pay Now!');
	}
	...
	function validate_ipn()
	{
		...
		if(!$fp)
		{
			...
			$this->last_error = "could not open the connection";
