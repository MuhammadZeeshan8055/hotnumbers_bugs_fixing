    ...
    <div class="row">
        ...
        <div class="col-md-8">
           <div class="pull-right">
               <table class="table layout-fixed text-left" style="width: 25em;">
                   <tbody>
                   <tr>
                       <th width="120">Item subtotal</th>
                       <td><span><?php echo currency_symbol ?></span><span id="item-subtotal">0.00</span></td>
                   </tr>
                   <tr>
                       <th>Shipping</th>
                       <td id="shipping-input" width="220">
                           <select id="select-order-shipping" name="order-shipping" onchange="calcSubtotal(this)" class="select2" disabled data-search="false">
                                       ...
                                       <div>
                                           <select onchange="applyCoupon(this)" name="coupon" data-search="false" class="select2" style="width: 200px;">
                                               ...
                                               foreach($coupons as $coupon) {
                                                   ...
                                                   $selected = '';
                                                   if(!empty($order_data['order_meta']['coupon_id'])) {
                                                       if($coupon->id === $order_data['order_meta']['coupon_id']) {
                                                           $selected = 'selected';
                                                       }
                                                   }
                                                   ?>
                   ...
                   <tr>
                       ...
                       <td>
                           <div class="input-group input_field">
                               <input type="number" step="0.01" min="0" id="order-discount" onchange="apply_discount(this)" class="form-control" style="width: 200px" value="<?php echo !empty($order_data['order_meta']['cart_discount']) ? $order_data['order_meta']['cart_discount']:0 ?>" name="discount">
                           </div>
                       </td>
                   </tr>
                   <tr>
                ...
                fetch('<?php echo admin_url() ?>ajax/address-tax-ajax',{
                    ...
                    if(res) {
                        ...
                        res.forEach((tax,idx)=>{
                            ...
                            ?>
                            selected = tax.name === '<?php echo $order_data['order_meta']['order_shipping_title'] ?>' ? 'selected':'';
                            <?php
                            } ?>
        ...
        await fetch(url).then(res=>res.json()).then(res=>{
            if(res && typeof res.attributes !== "undefined") {
                ...
                if(res.attributes.length) {
                    ...
                    parent.find('.option_total').html(price+'<input type="hidden" value="'+price+'" name="productTotal[]">');
        ...
        if(parseFloat($('select[name="coupon"]').val())) {
            ...
            if(discAmount) {
                if(discType === "percent") {
                    ...
                    discount = subtotal - discAmount;
                }
            }
            document.querySelector('#order-discount').value = discount;
        }

        subtotal -= document.querySelector('#order-discount').value;
